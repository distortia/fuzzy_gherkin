...
module FuzzyGherkin
    class Main
        ...
        def initialize(base_step = "I navigate to the foo page", threshold = 0.80)
        ...
        def parse_feature(feature_file = "example.feature")
          ...
          raise "FAILURE: File not found at #{feature_path}" if !File.file?(feature_path)
        ...
        def compare(comparing_step)
          puts "comparing_step #{comparing_step}"
            ...
            # 1.0 is a perfect match which means its a repeated step and we can ignore.
